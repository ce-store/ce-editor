...
var create = function (express) {
  ...
  function MemoryStore(options) {
    ...
    this.store.on('set', function(key) {
      ceStore.getStore(key, function(err) {
        if (err) {
          ceStore.createStore(key, function(err) {
            if (err) {
              console.log(err);
            } else {
              console.log('store added: ' + key);
            }
          });
        }
      });
    });
    this.store.on('expired', function(key) {
      ceStore.deleteStore(key, function(err) {
        if (err) {
          console.log(err);
        } else {
